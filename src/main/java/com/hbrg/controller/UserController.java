...
public class UserController {
    ...
    public String newUser(@Valid UserFormDto userFormDto, BindingResult bindingResult, Model model){
        ...
        try{
            Huser user = Huser.createUser(userFormDto, passwordEncoder);
    ...
    public String kakaoCallback(String code){    //Data를 리턴해주는 컨트롤러 함수
        ...
        params.add("client_id", "717132960bdd5b0e541acb17c327097d");
...
//        UUID garbagePassword = UUID.randomUUID();
        ...
        Huser kakaoUser = Huser.createUser(userFormDto, passwordEncoder);

        try {
            userService.validateDuplicateEmail(kakaoUser);
            // 중복되지 않은 이메일인 경우 회원가입 진행
            System.out.println("중복이메일 없음, 회원가입 완료");
            userService.saveUser(kakaoUser);
        } catch (IllegalStateException e) {
            // 이미 등록된 이메일이 있는 경우 처리
...
/*
        ...
        System.out.println("자동 로그인을 진행합니다11111.");
        SecurityContextHolder.getContext().setAuthentication(authentication);
*/

        UserDetails userDetails = userService.loadUserByUsername(kakaoEmail);
        Authentication authentication = new UsernamePasswordAuthenticationToken(userDetails,"hbrg1234", userDetails.getAuthorities());
        SecurityContextHolder.getContext().setAuthentication(authentication);
